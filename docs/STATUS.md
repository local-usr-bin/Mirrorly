...
> 最后更新：2026-09-13（T-10 端到端验收完成：推荐 PASS WITH ENVIRONMENTAL SKIPS，待全套复跑确认）

## 当前阶段

**MVP 开发进行中（10/10 任务全部实施，T-10 验收收尾中）** —— T-01~T-09 已最终验收（390 passed / 8 skipped / 0 failed @ 2487dd7→a31126b）。T-10 端到端验收已完成实施与执行（`tests/test_e2e.py` 11 用例真实 CLI E2E、`scripts/t10_perf_baseline.py` 性能基线、`docs/MVP_ACCEPTANCE.md` 验收报告），冻结四项验收标准全部有真实证据；推荐结论 **PASS WITH ENVIRONMENTAL SKIPS**（4 个文件级 symlink 用例环境权限限制跳过，目录级 junction 已真机通过；中断/retention 用例被宿主 safe-delete 护栏在最终确认运行拦截，runs 1–5 全绿）——**待配额刷新后全套 pytest 复跑确认即可最终定论**。
...
- [x] **T-10 端到端验收**：`tests/test_e2e.py`（11 用例，真实 CLI 子进程 + 真实 NTFS：主流程 A–F[init/backup#1/verify/变更/dry-run 零写入/backup#2/list]、restore[整快照字节级/—path 文件与子树/never/always]、损坏检测 exit 4、TR-5 真实 kill 续传、M10 身份[错误 serial exit 5 零写入 + 同卷新地址 SIMULATED 盘符漂移]、retention 多快照清理后 verify 全过）；`scripts/t10_perf_baseline.py` 性能基线两 workload（9.6 GB 混合/4 GB 大文件；unchanged 二次备份 0 字节写入、硬链接复用 10,006 文件、空间仅 +0.01 GB）；`docs/MVP_ACCEPTANCE.md` 验收报告 + 已知限制清单（含 M10 语义裁定：CLI_SPEC 冻结契约 = fail closed，US-3 自动重定位列 v2 候选）；reparse 真机补验：`_make_symlink` junction 回退使 4 个目录级用例真机通过、4 个文件级用例因无 symlink 特权（ERROR_PRIVILEGE_NOT_HELD）如实 skip；ruff 全绿；推荐 **PASS WITH ENVIRONMENTAL SKIPS**（中断/retention 用例被宿主 safe-delete 护栏在最终确认运行拦截、runs 1–5 全绿；全套 pytest 复跑确认见待办）

## 待办（建议优先级从高到低）

1. **T-10 收官**：配额刷新后全套 pytest 复跑（含 test_e2e.py 与 junction 回退后的 test_restore.py）确认 0 failed，MVP 验收定论（报告见 docs/MVP_ACCEPTANCE.md §12–13）
