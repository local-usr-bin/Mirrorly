...
- 全局选项：`--config <path>`（配置目录，默认 `./.mirrorly`）、`--task <name>`（指定任务；现有配置恰有一个任务时可省略，多个时必填）、`--quiet / --verbose`、`--no-color`、`--json`；可放在子命令前或后。`mirrorly --version` 显示当前 source/CLI version `0.1.0`（release preparation；`v0.1.0` tag / GitHub Release 尚未创建，历史 `v0.1.0-mvp` 保持冻结）。
